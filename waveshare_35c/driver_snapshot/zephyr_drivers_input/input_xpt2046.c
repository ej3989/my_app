...
static bool xpt2046_sample_and_report(struct xpt2046_data *data)
{
	...
	uint32_t raw_x = config->swap_xy ? meas.y : meas.x;
	uint32_t raw_y = config->swap_xy ? meas.x : meas.y;
...
#define XPT2046_INIT(index)                                                                        \
	static const struct xpt2046_config xpt2046_config_##index = {                              \
		...
		.swap_xy = DT_INST_PROP(index, swap_xy),                                           \
		.invert_x = DT_INST_PROP(index, invert_x),                                         \
		.invert_y = DT_INST_PROP(index, invert_y),                                         \
	};                                                                                         \
