...
static int confirm_running_ota_image(void)
{
	...
	if (err == 0) {
		LOG_INF("MCUboot OTA image confirmed after MQTT startup");
	} else {
		LOG_ERR("Failed to confirm MCUboot OTA image (err %d)", err);
	}
			       ...
			       const struct mqtt_evt *event)
{
	...
	case MQTT_EVT_CONNACK:
		...
		} else if (event->result == MQTT_BAD_USER_NAME_OR_PASSWORD) {
			LOG_ERR("MQTT login rejected: bad username or password");
		} else if (event->result == MQTT_NOT_AUTHORIZED) {
			LOG_ERR("MQTT login rejected: client is not authorized");
		} else {
